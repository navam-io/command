...
def test_stream_response(bedrock_instance):
    ...
    bedrock_instance.client.invoke_model_with_response_stream.return_value = (
        mock_response
    )
...
def test_stream_vision_response(bedrock_instance):
    mock_response = {
        "body": [
            {
                "chunk": {
                    "bytes": json.dumps({"delta": {"text": "This image"}}).encode()
                }
            },
    ...
    response = list(
        bedrock_instance.stream_vision_response(
            b"fake_image_data", "Describe this image"
        )
    )
