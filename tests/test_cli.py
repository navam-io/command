...
from command.cli import (ask, audit, cli, config, gather, id, init, intents,
...
def mock_config():
    return {
        "run": {
            "lookup-folder": "../command-vault/Code",
            "save-folder": "../command-vault/../../Apps",
        },
        "provider-model-mapping": {"provider1": ["model1", "model2"]},
        "test": {
            "ask": "test prompt",
            "vision": "vision prompt",
            "image-path": "../command-vault/Images/hackathon.jpg",
        },
        "ask": {
            "provider": "mock_provider",
            "prompts-folder": "../command-vault/Prompts",
        },
        "refer-section": {
            "provider": "mock_provider",
            "lookup-folder": "../command-vault/Raw",
        },
        "intents": {
            "provider": "mock_provider",
            "lookup-folder": "../command-vault/Intents",
            "save-folder": "../command-vault/Embeds",
        },
        "merge": {
            "lookup-folder": "../command-vault/Posts",
        ...
        "vision": {
            "provider": "mock_provider",
            "model": "vision_model",
            "lookup-folder": "../command-vault/Gather/images",
            "system": "System prompt",
            "save": True,
            "save-folder": "../command-vault/Vision",
        },
...
def mock_provider_instance():
    instance = MagicMock()
    instance.ask.return_value = "../command-vault/Posts/response.md"
...
@patch("command.cli.configure.load_config")
@patch("command.cli.utils.get_provider_instance")
@patch("command.cli.markdown.file_select_paginate")
@patch("command.cli.code.process_markdown_file")
def test_run(
    ...
    mock_file_select.return_value = "../command-vault/Code/selected_file.md"

    result = runner.invoke(run)
    assert result.exit_code == 0
    mock_file_select.assert_called_once_with("../command-vault/Code")
    mock_process_markdown.assert_called_once_with(
        "../command-vault/Code/selected_file.md",
        app_folder="../command-vault/../../Apps",
    )


@patch("command.cli.configure.load_config")
@patch("command.cli.configure.save_config")
@patch("command.cli.utils.get_provider_instance")
@patch("command.cli.metrics.save_test_summary")
def test_test(
    mock_save_summary,
    mock_get_provider,
    mock_save_config,
...
@patch("command.cli.configure.load_config")
@patch("command.cli.utils.get_provider_instance")
@patch("command.cli.markdown.parse_markdown_sections")
@patch("command.cli.markdown.diff")
def test_validate(
    mock_diff,
    mock_parse,
    mock_get_provider,
    ...
    ), patch("command.cli.click.prompt", return_value=1):
        result = runner.invoke(validate, ["-d", "../command-vault/Intents/test_doc.md"])
        assert result.exit_code == 0
...
@patch("command.cli.markdown.file_select_paginate")
@patch("command.cli.markdown.parse_markdown_sections")
@patch("command.cli.markdown.intent_select_paginate")
@patch("command.cli.click.confirm")
@patch("command.cli.markdown.update_markdown_with_response")
def test_intents_existing_response(
    ...
    mock_load_config.return_value = mock_config
    mock_get_provider.return_value = mock_provider_instance
    mock_file_select.return_value = "../command-vault/Intents/intents.md"
    mock_parse.return_value = [("Intent1", "Prompt1")]
...
@patch("command.cli.utils.get_provider_instance")
@patch("command.cli.markdown.file_select_paginate")
@patch("command.model_vision.images.resize_image")
@patch("command.model_vision.images.display_image")
@patch("command.model_vision.requests.get")
@patch("command.cli.configure.has_vision_capability", return_value=True)
def test_vision_with_url(
    ...
    with patch(
        "command.model_vision.tempfile.NamedTemporaryFile"
    ) as mock_temp_file, patch("command.cli.os.unlink"), patch(
        "command.cli.shutil.copyfileobj"
    ), patch(
...
@patch("command.cli.configure.load_config")
@patch("command.cli.markdown.merge_docs")
...
@patch("command.cli.configure.edit_config")
...
@patch("command.cli.gather_utils.article")
...
@patch("command.cli.markdown.split_text_by_tokens")
def test_split(mock_split, runner):
    result = runner.invoke(split, ["../command-vault/Raw/file.txt"])
    assert result.exit_code == 0
    mock_split.assert_called_once_with("../command-vault/Raw/file.txt")


@patch("command.cli.metrics.read_yaml_files")
@patch("command.cli.metrics.process_data")
@patch("command.cli.metrics.display_trends")
