...
from command.code import _open_vite_server, process_markdown_file
...
@pytest.mark.parametrize(
    "file_content,expected_app_folder",
    [
        (
            "# Install script\n\n```bash\nmkdir test_app\ncd test_app\n```\n# File: test.txt\n\n```\nTest content\n```",
            "test_app",
...
def test_process_markdown_file(file_content, expected_app_folder, mock_console):
    with patch("builtins.open", mock_open(read_data=file_content)) as mock_file, patch(
    ...
    ), patch("os.makedirs") as mock_makedirs, patch("os.chmod") as mock_chmod, patch(
        "subprocess.run"
        ...
        mock_file.assert_any_call(
            f"/current/dir/apps/{expected_app_folder}_install_script.sh", "w"
        )

        # Check if chmod was called for the install script
        mock_chmod.assert_called_with(
            f"/current/dir/apps/{expected_app_folder}_install_script.sh", 0o755
        )
        ...
        mock_file.assert_any_call(
            f"/current/dir/apps/{expected_app_folder}/test.txt", "w"
        )
...
"""
    ...
    with patch("builtins.open", mock_open(read_data=file_content)) as mock_file, patch(
        "os.path.exists", side_effect=[False, False, True, True]
    ), patch("os.makedirs") as mock_makedirs, patch("os.chmod") as mock_chmod, patch(
        "subprocess.run"
    ) as mock_subprocess_run, patch(
        "subprocess.Popen"
        ...
        mock_subprocess_run.assert_called_with(
            ["/bin/bash", "/current/dir/apps/cleanup_script.sh"], check=True
        )
...
"""
    ...
    with patch("builtins.open", mock_open(read_data=file_content)):
        with pytest.raises(
            ValueError, match="Install script not found in the markdown file"
        ):
            process_markdown_file("test.md", "apps")
...
"""
    ...
    with patch("builtins.open", mock_open(read_data=file_content)) as mock_file, patch(
        ...
        mock_subprocess_run.assert_has_calls(
            [
                call(
                    ["/bin/bash", "/current/dir/apps/test_app_install_script.sh"],
                    check=True,
                    cwd="/current/dir/apps",
                ),
