...
def gemini_instance():
    with patch("google.generativeai.configure"), patch(
        "os.environ.get", return_value="fake_api_key"
    ):
        yield Gemini()
...
def test_init_no_api_key():
    with patch("os.environ.get", return_value=None), pytest.raises(
...
def test_create_request_data_with_image(gemini_instance):
    ...
    assert request_data["contents"][0] == {
...
async def test_stream_vision_response(gemini_instance):
    ...
    with patch("google.generativeai.GenerativeModel", return_value=mock_model):
        response = gemini_instance.stream_vision_response(
