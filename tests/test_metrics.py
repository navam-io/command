...
from command.metrics import (count_tokens, create_sparkline, display_trends,
                             generate_yaml_data, process_data, read_yaml_files,
                             save_test_summary, save_to_yaml)
...
@patch("command.metrics.generate_yaml_data")
@patch("command.metrics.save_to_yaml")
def test_save_test_summary(mock_save_to_yaml, mock_generate_yaml_data):
    mock_generate_yaml_data.return_value = {"test": "data"}
    save_test_summary(
        "provider", "model", "config", "prompt", "status", "details", 1.5, 100
    )
...
def test_read_yaml_files(mock_yaml_load, mock_file, mock_listdir):
    mock_listdir.return_value = [
        "test_summary_20230101.yml",
        "test_summary_20230102.yml",
    ]
...
def test_process_data():
    data = {
        "20230101": {
            "2023-01-01T12:00:00": {
                "provider1": {
                    "model1": {"config1": {"response_time": 1.5, "token_count": 100}}
                }
            }
        }
    }

    with patch("command.metrics.datetime") as mock_datetime:
